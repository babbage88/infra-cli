...
func (rgi *RemoteGarageInstaller) EnsureInstalledAndConfigured(cfg GarageNodeConfig) error {
	...
	if err := rgi.verifyGarageStatus(cfg); err != nil {
		return err
	}
...
case "$arch" in
  ...
  armv6l|armv7l)
    ...
    ;;
  *)
    echo "unsupported architecture for official Garage binary download: $arch" >&2
    exit 1
    ;;
esac
...
func (rgi *RemoteGarageInstaller) restartService() error {
	restartScript := `set -e
if sudo systemctl restart garage >/dev/null 2>&1; then
  exit 0
fi
if sudo systemctl start garage >/dev/null 2>&1; then
  exit 0
fi
